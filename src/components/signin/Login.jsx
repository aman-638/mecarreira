  ...
  const formSubmit = (e) => {
    ...
    axios.post(`https://restapi.mecarreira.com/accounts/login/`,formData).then((res) => {
      if(res.data.success){
        ...
        localStorage.setItem("meca",JSON.stringify(formData));
        navigate('/otp');
      }else{
        alert(res.data.message);
      }
          ...
          <div className='w-[380px] rounded-xl'>
             ...
             <div className='bg-[#212435] rounded-b-xl h-[400px] md:h-[500px]'>
                ...
                <input id='password' onChange={changeHandler} className=' w-[80%] flex m-auto p-3 rounded font-semibold bg-[#2c2f45] text-[#a0aaaf]  outline-none' type="password" placeholder='Enter Password' /><br></br>
                 ...
                 <input className=' cursor-pointer w-[80%] flex justify-center m-auto rounded-xl text-[#68c20b] border-[#68c20b] border-2 py-3 px-8 font-semibold bg-transparent hover:text-[#29990e] hover:border-[#29990e] my-6' type="submit" value="Sign In" />
                </form>
